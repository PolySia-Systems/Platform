...
def test_registry_registers_stable_version_with_unrated_performance() -> None:
    ...
    assert registry.get(definition.strategy_id, "0.1.0") == definition
    assert registry.list() == (definition,)
    assert registry.performance(definition.strategy_id, "0.1.0").score_status == "unrated"
    assert (
        registry.performance(definition.strategy_id, "0.1.0").evidence_sufficiency
        == "insufficient"
    )
