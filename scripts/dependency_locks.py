...
CANONICAL_LOCK_HEADER = """\
#
# This file is generated by python scripts/dependency_locks.py from pyproject.toml.
# Do not edit pin rows by hand.
#
"""
...
def _sanitize_generated_lock(text: str) -> str:
    ...
    if unexpected:
        raise RuntimeError(
            f"generated lock includes local project pins: {sorted(unexpected)}"
        )
    return text.replace("\r\n", "\n")


def _canonicalize_lock(text: str) -> str:
    sanitized = _sanitize_generated_lock(text)
    body: list[str] = []
...
def generate_pip_locks(
    ...
    *,
    ...
    _write_text(runtime_path, _canonicalize_lock(_read_text(runtime_path)))
    _run(
        _pip_compile_command(
            extra="dev",
    ...
    _write_text(dev_path, _canonicalize_lock(_read_text(dev_path)))
    ...
    if any(
        name in runtime_pins
        for name in ("pytest", "ruff", "mypy", "pip-tools", "pip-audit")
    ):
    ...
    with tempfile.TemporaryDirectory(prefix="polysia-lock-check-") as raw:
        ...
        for relative in (RUNTIME_LOCK, DEV_LOCK):
            ...
            if committed != generated:
                committed_pins = parse_lock_pins(committed)
                generated_pins = parse_lock_pins(generated)
                pin_diff = changed_lock_packages(committed, generated)
                raise RuntimeError(
                    f"{relative} is not the deterministic pip-compile output of "
                    "pyproject.toml. "
                    f"pin changes={pin_diff or 'none'}; "
                    f"committed_pins={len(committed_pins)} generated_pins={len(generated_pins)}"
                )
