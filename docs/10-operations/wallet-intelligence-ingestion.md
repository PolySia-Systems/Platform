...
| Stage 4B transient source or market-read failure | Watermark and persistent portfolio remain last-known-good; failed poll is sanitized and the persistent worker retries on its normal interval | Inspect the atomic `continuous-shadow.json` artifact and source telemetry; do not query the live database |
| Stage 4B persistence, lease-integrity, or unexpected failure | Previous durable state remains; worker fails closed for systemd recovery | Inspect the sanitized stage, ledger, and duplicate count before accepting recovery |
| Stage 4B accounting or duplicate-processing invariant failure | Publication rolls back; checkpoint, watermark, and financial state stay last-known-good; poll/experiment record `accounting_blocked` or `duplicate_processing`; worker exits 0 and does not systemd-restart | Do not auto-repair. Restore a verified recovery bundle or repair offline, revalidate Decimal ledger identity, then start the worker explicitly |
| Stage 4A overlap causes Stage 4B `sqlite_busy` | Initialization, lease, state loading, or poll persistence rolls back, or post-poll health refresh keeps the prior atomic artifact; persistent worker continues | Confirm the sanitized `initialize`, `load_state`, lease, `persist`, or `report_health` stage, unchanged `NRestarts`, balanced ledger, and zero duplicate processing; the next normal interval retries |
| Stage 4B ledger mismatch or finalized experiment with positions | Health `critical`; accounting invariants also block the next poll | Disable only the Stage 4B timer, preserve DB/backup, restore or repair, then revalidate before restart |
