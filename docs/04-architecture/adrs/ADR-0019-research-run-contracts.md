...
disk, memory, and a local host admission lock immediately before start/resume
and verify. The worker is the only writer of Manifest state. Clients persist a
stop command (id, content fingerprint, expected revision, disposition) and a
stop-request file; repeating the same command returns the recorded
disposition. Large restore and analysis copies use an operation-owned scratch
directory beside the bundle or database, with filesystem preflight.
...
changes, and tampered Plans fail before T0. Old Runs without `run-plan.json`
remain readable; a Plan may be written additively from frozen Manifest fields.
Rollback is revert of this ADR and the contract/command/scratch modules.
