...
flowchart TB
  ...
  Outer["Outer services: execution, storage, reconciliation, bus, orderbook, features, portfolio, risk, strategies\n[CURRENT]"]:::current
  ...
  Tests["Architecture tests enforce all three boundaries\n[CURRENT]"]:::safe
...
Architecture tests prohibit domain/application adapter or SDK imports, strategy/storage adapter imports, and official SDK imports outside `adapters/polymarket`. Actual outer services may depend on adapters where operationally required.
...
`src/polysia/domain/`, `src/polysia/application/`, `src/polysia/adapters/`, `src/polysia/strategies/`, `src/polysia/storage/`, `docs/00-governance/registers/technical-debt.md`
